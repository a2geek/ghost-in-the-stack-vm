...
import a2geek.ghost.target.mos6502.ExpressionGenerator.TempSupplier;
...
public class ExpressionGenerators {
    ...
    public static TempSupplier symbolSupplier(final Symbol symbol) {
        return new TempSupplier() {
            @Override
            public Terminal get() {
                return symbolReference(symbol);
            }
            @Override
            public void free(Terminal temp) {
                // TODO free only if Terminal is a temp.
            }
        };
    }
    public static TempSupplier terminalSupplier(final Terminal term) {
        return new TempSupplier() {
            @Override
            public Terminal get() {
                return term;
